...
pub fn sat_benchmark(c: &mut Criterion) {
    for (group_name, config) in create_sat_configs() {
...
pub fn radio_benchmark(c: &mut Criterion) {
    for (group_name, config) in create_radio_configs() {
        let mut group = c.benchmark_group(group_name);
        group.sample_size(10);
        group.bench_with_input(
...
pub fn nqueens_benchmark(c: &mut Criterion) {
    for (group_name, config) in create_nqueens_configs() {
        let mut group = c.benchmark_group(group_name);
        group.sample_size(10);
        group.bench_with_input(
...
fn create_sat_configs() -> Vec<(String, Config)> {
...
fn create_radio_configs() -> Vec<(String, Config)> {
    let mut configs: Vec<(String, Config)> = Vec::new();
    let pop_config = PopConfig {
        dim: 10,
        pop_size: 30,
    ...
    configs.push((format!("RADIO GENS {} PARALLEL {}", 150, PARALLEL), config));
    configs
}

fn create_nqueens_configs() -> Vec<(String, Config)> {
    let mut configs: Vec<(String, Config)> = Vec::new();
